...
def compute_critic_loss(
    ...
    online_actions = q_values[0].max(dim=-1)[1]

    # Action evaluation using the target network
    target_q_values = q_target[1].gather(dim=-1, index=online_actions.unsqueeze(-1)).squeeze(-1)

    # Computing the target values
    target = reward[1] + discount_factor * target_q_values * must_bootstrap[1]

    # Computing the Q-values of taken actions
...
@hydra.main(
    config_path="./",
    config_name="ddqn_lunarlander.yaml",
