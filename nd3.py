...
# Create the ND3 Agent
def create_nd3_agent(cfg, train_env_agent, eval_env_agent, n_critics):
    obs_size, act_size = train_env_agent.get_obs_and_actions_sizes()

    # Create n_agents Q-networks and their target counterparts
    critics = []
    targets = []
    for i in range(n_critics):
        critics.append(ContinuousQAgent(
            obs_size,
            cfg.algorithm.architecture.critic_hidden_size,
            act_size,
            seed=cfg.algorithm.seed.q,
        ).set_name("critic{}".format(i)))
        targets.append(copy.deepcopy(critics[i]).set_name("target-critic{}".format(i)))
    ...
    return train_agent, eval_agent, actor, critics, targets, target_actor
...
def setup_optimizers(cfg, actor, critics):
    ...
    for i in range(len(critics)):
        ...
        critics_optimizers.append(get_class(cfg.critic_optimizer)(critics[i].parameters(), **critic_optimizer_args))

    return actor_optimizer, critics_optimizers


def compute_critic_loss(cfg, reward, must_bootstrap, q_values_list, target_q_values):
    ...
    for i in range(len(q_values_list)):
        if critic_loss is None:
            critic_loss = mse(target, q_values_list[i].squeeze(-1))
        else:
            critic_loss += mse(target, q_values_list[i].squeeze(-1))
...
def run_nd3(cfg, logger, trial=None):
    best_reward = float("-inf")

    # 2) Create the environment agent
    train_env_agent, eval_env_agent = get_env_agents(cfg)

    # 3) Create the ND3 Agent
    (
        train_agent,
        eval_agent,
        actor,
        ...
        targets_critics,
        target_actor
    ) = create_nd3_agent(cfg, train_env_agent, eval_env_agent, cfg.algorithm.architecture.n_critics)
    ag_actor = TemporalAgent(actor)
    # ag_target_actor = TemporalAgent(target_actor)
    q_agents = []
    targets_q_agents = []
    for i in range(cfg.algorithm.architecture.n_critics):
        q_agents.append(TemporalAgent(critics[i]))
        targets_q_agents.append(TemporalAgent(targets_critics[i]))

    train_workspace = Workspace()
    rb = ReplayBuffer(max_size=cfg.algorithm.buffer_size)

    # Configure the optimizer
    actor_optimizer, critics_optimizers = setup_optimizers(cfg, actor, critics)
    nb_steps = 0
    tmp_steps = 0
    ...
    while nb_steps < cfg.algorithm.n_steps:
        ...
        for _ in range(cfg.algorithm.optim_n_updates):
            ...
            if nb_steps > cfg.algorithm.learning_starts:
                ...
                # Critic update
                # compute qs_values: at t, we have Q(s,a) from the (s,a) in the RB
                # the detach_actions=True changes nothing in the results
                qs_values = []
                for i in range(cfg.algorithm.architecture.n_critics):
                    q_agents[i](rb_workspace, t=0, n_steps=1, detach_actions=True)
                    qs_values.append(rb_workspace["critic{}/q_values".format(i)])
                ...
                with torch.no_grad():
                    ...
                    # compute Q_values from all target critics: at t+1 we have Q(s_{t+1}, \pi(s_{t+1})
                    targets_qs_values = []
                    for i in range(cfg.algorithm.architecture.n_critics):
                        targets_q_agents[i](rb_workspace, t=1, n_steps=1, detach_actions=True)
                        targets_qs_values.append(rb_workspace["target-critic{}/q_values".format(i)])

                    # q_agent(rb_workspace, t=1, n_steps=1)
                # Use the minimum of the N target Q-values
                post_q_values = torch.min(torch.as_tensor(targets_qs_values), axis=0)

                # Compute critic loss
                critic_loss = compute_critic_loss(
                    cfg, reward, must_bootstrap, qs_values, post_q_values[1]
                )
                logger.add_log("critic_loss", critic_loss, nb_steps)
                for i in range(cfg.algorithm.architecture.n_critics):
                    critics_optimizers[i].zero_grad()

                critic_loss.backward()

                for i in range(cfg.algorithm.architecture.n_critics):
                    torch.nn.utils.clip_grad_norm_(
                        critics[i].parameters(), cfg.algorithm.max_grad_norm
                    )
                    critics_optimizers[i].step()
                ...
                if nb_steps % cfg.algorithm.policy_delay == 0:
                    ...
                    q_agents[0](rb_workspace, t=0, n_steps=1)
                    # and we back-propagate the corresponding loss to maximize the Q values
                    q1_values = rb_workspace["critic0/q_values"]  # Use one of the Q-values for actor loss
                    ...
                    for i in range(cfg.algorithm.architecture.n_critics):
                        soft_update_params(critics[i], targets_critics[i], cfg.algorithm.tau)
...
@hydra.main(
    config_path="./",
    # config_name="nd3_pendulum.yaml",
    config_name="nd3_pendulum_optuna.yaml",
)  # , version_base="1.3")
def main(cfg_raw: DictConfig):
    torch.random.manual_seed(seed=cfg_raw.algorithm.seed.torch)

    if "optuna" in cfg_raw:
        launch_optuna(cfg_raw, run_nd3)
    else:
        logger = Logger(cfg_raw)
        run_nd3(cfg_raw, logger)
