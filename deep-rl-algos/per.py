...
class PrioritizedReplayBuffer(ReplayBuffer):
    def __init__(self, max_size, alpha, beta, device=torch.device("cpu")):
        super().__init__(max_size, device)
        self.alpha = alpha
        self.beta = beta
        self.priorities = torch.zeros(max_size, device=device)

    def put(self, workspace, priorities):
        super().put(workspace)
        indexes = torch.arange(self.position) + self.position - len(priorities)
        indexes = indexes % self.max_size  # Handle wrap-around
        self.priorities[indexes] = torch.tensor(priorities, device=self.device)

    def get_shuffled(self, batch_size):
        if self.size() == 0:
            return Workspace()

        priorities = self.priorities[:self.size()].pow(self.alpha)
        probs = priorities / priorities.sum()
        who = torch.multinomial(probs, batch_size, replacement=True)
        weights = (self.size() * probs[who]).pow(-self.beta).to(self.device)
        weights /= weights.max()
        ...
        return workspace, weights, who
    
    def update_priorities(self, indices, priorities):
        self.priorities[indices] = torch.as_tensor(priorities, device=self.device)
...
def compute_critic_loss(
    discount_factor, reward, must_bootstrap, action, q_values, weights, q_target=None
):
    """Compute critic loss
    Args:
        ...
        weights (torch.Tensor): a (B) tensor containing importances of each example of the batch used
    ...
    td_errors = (qvals - target).detach().abs().cpu().numpy()
    weighted_mse_loss = ((qvals - target) ** 2 * weights).mean()
    return weighted_mse_loss, td_errors
...
def run_dqn(cfg, logger, trial=None):
    ...
    # 4) Create the prioritized replay buffer
    replay_buffer = PrioritizedReplayBuffer(
        max_size=cfg.algorithm.buffer.max_size,
        alpha=cfg.prioritized_replay.alpha,
        beta=cfg.prioritized_replay.beta,
    ...
    while nb_steps < cfg.algorithm.n_steps:
        ...
        for _ in range(cfg.algorithm.optim_n_updates):
            ...
            sampled_trans_ws, weights, indices = replay_buffer.get_shuffled(
            ...
            if replay_buffer.size() > cfg.algorithm.buffer.learning_starts:
                # Compute critic loss
                critic_loss, td_errors = compute_critic_loss(
                ...
                replay_buffer.put(transition_workspace, td_errors)
        ...
        # Beta annealing
        replay_buffer.beta *= cfg.prioritized_replay.beta_annealing
...
@hydra.main(
    config_path="./",
    config_name="dqn_lunarlander.yaml",
