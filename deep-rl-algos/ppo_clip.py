...
def compute_advantage(cfg, reward, must_bootstrap, v_value):
    # Compute temporal difference with GAE(generalized advantage estimation),T= must_bootstrap.shape[0]
        # N-steps= must_bootstrap.shape[0]
    advantage = gae(
        v_value,
        reward,
        must_bootstrap,
...
def compute_critic_loss_mc(cfg, advantage, must_bootstrap, critic):
    # To be completed...
    #print(critic.shape, must_bootstrap.shape)
    diacounted_reward=advantage.clone()
    for i in range(-1,-len(advantage),-1):
        diacounted_reward[i] = diacounted_reward[i] + cfg.algorithm.discount_factor*diacounted_reward[i+1]* must_bootstrap[i].int()
    critic_loss = ((diacounted_reward-critic)**2).sum(axis=0).mean()
...
def run_ppo_clip(cfg, logger, trial=None):
    ...
    while nb_steps < cfg.algorithm.n_steps:
        ...
        for opt_epoch in range(cfg.algorithm.opt_epochs):
            ...
            # advantage= compute_policy_loss_mc(cfg, advantage, must_bootstrap, critic)

            policy_advantage = advantage.detach()[1]
            #print(policy_advantage.shape, ratios.shape)
            ...
            loss_entropy = -cfg.algorithm.entropy_coef * entropy_loss  # permet variance dans exploration, favorise policy stochastic
...
@hydra.main(
    config_path="./configs/",
    #config_name="ppo_lunarlander_optim.yaml",
